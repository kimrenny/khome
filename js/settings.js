document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadToggleState(toggleId) {
        const state = localStorage.getItem(toggleId);
        if (state !== null) {
    ...
    const savedGPTModel = localStorage.getItem("savedGPTModel");
    if(savedGPTModel === "gpt-3.5-turbo"){
        const radioButtonGPT3_5 = document.querySelector('input[value="GPT-3.5"]');

        if(radioButtonGPT3_5){
            radioButtonGPT3_5.checked = true;
        }
    }else if(savedGPTModel === "gpt-4"){
        const radioButtonGemini = document.querySelector('input[value="GPT-4"]');

        if(radioButtonGemini){
            radioButtonGemini.checked = true;
        }
    }
    ...
    radioButtons.forEach(function(button){
        button.addEventListener('change', function(){
            if(this.value == "GPT-3.5"){
                localStorage.setItem("savedGPTModel", "gpt-3.5-turbo");
            }else if(this.value == "GPT-4"){
                localStorage.setItem("savedGPTModel", "gpt-4");
            }
        });
    });
