document.addEventListener("DOMContentLoaded", function(){
    ...
    function saveTimerOrAlarm(name, time, type){
        if(!name || !time){
            console.error("Name or time is missing");
            return;
        }
        const itemKey = type === 'timer' ? 'timers' : 'alarms';
        const existingItems = JSON.parse(localStorage.getItem(itemKey)) || [];
        existingItems.push({name, time});
    ...
    function loadTimersAndAlarms(){
        ...
        if(!timersString || !alarmsString){
            console.error("Timers or alarms data is missing in localStorage");
            return;
        }
        ...
        const timers = JSON.parse(timersString) || [];
    ...
    */
    ...
    const existingAlarms = document.querySelectorAll(".alarm-container");
    ...
    function startTimer(timeString){
        ...
        const savedAlarmSound = localStorage.getItem("savedAlarmSound");
        if(savedAlarmSound){
    ...
    function startAlarm(alarmTime){
        ...
        function updateAlarm(alarmProgress, circumference, timerText){
            const progressPercentage = ((duration - timer) / duration) * 100;
            const offset = circumference * (progressPercentage / 100);
